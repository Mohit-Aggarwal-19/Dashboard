...
export class DummyComponent implements OnInit{
  ...
  // ngOnInit() {
  //   const val=this.getDataFromAPI();
  //   this._readMapDataService.getJSONData().subscribe(data=>
  //     {
  ...
  //         {
  //           // console.log(elementJSON.properties["hc-key"],":",elementJSON.properties["name"]);
  //           this.chartOptions.series[0].data.forEach((elementMapData: any)=>
  //           {
  //             if(elementMapData[0]===elementJSON.properties["hc-key"])
  //             {
  //               console.log(val.Deaths[elementJSON.properties["name"]]);

  //               console.log(elementMapData[0],":",elementJSON.properties["name"])
  //             }
  //           });
  //         });
  //         this.chartOptions.series[0].data.forEach((elementMapData: any)=>
      ...
      )).subscribe(data=>
            {
                ...
                data.objects.default.geometries.forEach((elementJSON: { properties: { [x: string]: any; }; }) => 
                {
                  ...
                  this.chartOptions.series[0].data.forEach((elementMapData: any)=>
                  {
                    if(elementMapData[0]===elementJSON.properties["hc-key"])
                    {
                      console.log(elementMapData[0],":",elementJSON.properties["name"],":",this.Deaths[elementJSON.properties["name"]])
                    }
                  });
                });
                this.chartOptions.series[0].data.forEach((elementMapData: any)=>
  ...
  //           "Case Fatality Rate":this.Case_Fatality_Ratio,
  //           "Incident Rate":this.Incident_Rate}
